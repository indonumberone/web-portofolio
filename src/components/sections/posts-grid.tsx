...
export function PostsGrid() {
  ...
  useEffect(() => {
    async function fetchPosts() {
      try {
        ...
        console.log("Sample post tags:", data[0]?.tags);
  ...
  const filteredPosts =
    selectedTag === null
      ? posts
      : posts.filter((post) => {
          const hasTag = post.tags.some((tag) => tag.name === selectedTag);
          // Debug logging
          ...
          <div className="flex flex-wrap gap-2 mb-2 sm:hidden">
            {allTags.map((tag) => (
              <button
                ...
                onClick={() => {
                  console.log("Selected tag (mobile):", tag);
                  setSelectedTag(tag);
                }}
              >
                {tag}
          ...
          {showDesktopTags &&
            allTags.map((tag) => (
              <button
                ...
                onClick={() => {
                  console.log("Selected tag (desktop):", tag);
                  setSelectedTag(tag);
                }}
              >
                {tag}
